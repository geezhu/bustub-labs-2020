...
TEST_F(GradingExecutorTest, SimpleUpdateTest) {
  ...
  auto txn_mgr = GetTxnManager();
  auto lock_manager = GetLockManager();
  auto exec_ctx = std::make_unique<ExecutorContext>(txn, GetCatalog(), GetBPM(), txn_mgr, lock_manager);
...
TEST_F(GradingExecutorTest, SchemaChangeSeqScan) {
  ...
  const Schema *out_schema2;
  {
    ...
    auto &schema2 = table_info2->schema_;
    auto outA = MakeColumnValueExpression(schema2, 0, "colA");
    auto outB = MakeColumnValueExpression(schema2, 0, "colB");
