 ...
 */
#define LEAF_PAGE IndexPageType::LEAF_PAGE
#define INTERNAL_PAGE IndexPageType::INTERNAL_PAGE
bool BPlusTreePage::IsLeafPage() const { return page_type_ == LEAF_PAGE; }
 ...
 */
// max kv count for leaf
#define LEAF_MAX_KV (max_size_ - 1)
// max kv count for internal,include kv[0]
#define INTERNAL_MAX_KV (max_size_)
int BPlusTreePage::GetMinSize() const {
  if (IsRootPage()) {
    return IsLeafPage() ? 1 : 2;
  }
  return ceil((IsLeafPage() ? LEAF_MAX_KV : INTERNAL_MAX_KV) / 2.0);
}
